...
import { AiOutlineClose, AiOutlineDown, AiOutlineSetting, AiOutlineQuestionCircle, AiOutlineLeft } from 'react-icons/ai'
...
import { displaySwapETHforToken, displayTokenModal, showTransactionSettingsModal } from '../components/Features/ModalSlice';
...
function Swap() {
    const {walletModal, transactionSettingsModal, tokenModal, manageModal, swapETHModal} = useSelector((store) => store.modal)
    ...
    { !confirmSwap   ?
        <div>
            <section className='w-full max-w-[464px] sm:w-[464px] h-[432px] px-3 sm:px-[24px] pt-[24px] pb-[32px] bg-[#152F30] rounded-[8px] m-auto'>
                ...
                <div>
                    <div>
                        ...
                        <div className='flex justify-between relative'>
                            ...
                            />
                            <i className={`absolute left-[45%] sm:left-[210px] top-1/2 -translate-y-1/2 cursor-pointer border w-[20px] h-[20px] rounded-full ${swapInputs.from ? 'flex items-center justify-center' : 'hidden'}`} onClick={clearSwapInputFrom}>
                                <AiOutlineClose />
                            </i>
                    ...
                    <div>
                        ...
                        <div className='flex justify-between'>
                            ...
                            />
                            { swapInputs.to === '' ?
                                <button className='flex items-center text-[#011718] bg-[#69CED1] w-[170px] h-[48px] justify-center rounded-[100px] hover:opacity-70 cursor-pointer' onClick={() => dispatch(displayTokenModal())}>Select a Token <i className='ml-2 text-white'><AiOutlineDown /></i></button>
                                 :
                                <div className='flex items-center h-[48px] justify-center cursor-pointer'>
                                     <img src={dash} alt="dash" /> 
                                     <span className='ml-2'>Dash </span>
                                    <i className='ml-2 text-white'><AiOutlineDown /></i>
                                </div>
                            }
                        </div>
                        <p>Balance: -</p>
...

                {
                    ...
                    <button className='text-[#011718] h-[48px] w-full max-w-[384px] sm:w-[384px] bg-[#69CED1] block m-auto mt-7 rounded-[100px]' onClick={displayConfirmSwap}>Swap</button>
                     :
        ...
        <section className='w-full max-w-[464px] sm:w-[464px] h-[568px] px-3 sm:px-[24px] pt-[24px] pb-[32px] bg-[#152F30] rounded-[8px] m-auto' >
            <header className='flex items-center mb-5 justify-between'>
                <i onClick={returnToSwap} className="cursor-pointer"><AiOutlineLeft /></i>
                <h3 className=''>Confirm Swap</h3>
                <div className=' cursor-pointer' onClick={() => dispatch(showTransactionSettingsModal())}><AiOutlineSetting /></div>
            </header>
            ...
            <div>
                <div className='mb-8'>
                    ...
                    <div className='flex justify-between relative'>
                        <p className='w-fit sm:w-[230px] bg-[#152F30] outline-none text-[20px] sm:text-[34px]'>{swapInputs.from}</p>
                        <div className='flex items-center cursor-pointer'>
                            <img src={blackDiamond} alt="black-diamond" className='w-[40px] h-[40px]'/>
                            <span className='text-[18px] ml-2'>ETH</span>
                        </div>
            ...
            <div>
                <h3>Swap to</h3>
                <div className='flex justify-between'>
                    <p className='w-[50%] sm:w-[230px] pr-2 bg-[#152F30] outline-none text-[20px] sm:text-[34px]'>{swapInputs.to}</p>
                    <div className='flex items-center h-[48px] justify-center cursor-pointer'>
            ...
            <p className='text-[#DCDCDC] my-5'>Output is estimated. You will receive at least 938.5 AMPL or the transaction will revert.</p>
            ...
            <button className='text-[#011718] h-[48px] w-full max-w-[384px] sm:w-[384px] bg-[#69CED1] block m-auto mt-7 rounded-[100px]' onClick={() => dispatch(displaySwapETHforToken())}>Confirm Swap</button>
        </section>
    }
    </div>
  )

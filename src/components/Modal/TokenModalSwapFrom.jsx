...
function TokenModalSwapFrom() {
  const { defaultTokenSwapTo } = useSelector((state) => state.token);

  const filteredList = TokenList.filter(
    (token) => defaultTokenSwapTo.token !== token.token
  );
  ...
  function selectThisDefaultToken(token) {
    dispatch(selectDefaultSwapFrom(token));
      ...
      <div className="bg-[#152F30] w-full max-w-[464px] sm:w-[464px] h-[564px] p-4 absolute top-8 left-1/2 -translate-x-1/2 z-50 rounded-lg flex flex-col">
        <header className="flex items-center mb-5">
          ...
          <div
            ...
            onClick={() => dispatch(hideTokenModalSwapFrom())}
          >
            <AiOutlineClose />
          </div>
        </header>
      ...
      <p className="ml-auto">
        {searchToken.filteredToken.length > 0 ? (
          <button
            ...
            onClick={() => importForSwapFrom(token.id)}
          >
            Import
          </button>
        ) : (

...
function LiquidityMain() {
    ...
    function toggleDisplayFullPool(id){
        ...
        setPools(prevValue => prevValue.map(pool => pool.id === id ? {...pool, liquidity: !pool.liquidity} : pool))
    }
  return (
    <section className='w-full max-w-[586px] sm:w-[586px] mx-auto px-3 sm:px-0'>
        ...
        <div className='flex items-center justify-between mt-9'>
            ...
            <div className='flex items-center text-[12px] sm:text-[16px]'>
                <button className='text-[#69CED1] sm:w-[127px] h-[36px] sm:h-[48px] bg-[#1B595B] px-2 rounded-[20px] sm:rounded-[100px] hover:brightness-125' onClick={() => dispatch(showCreateAPair())}>Create a Pair</button>
                <button className='text-[#011718] bg-[#69CED1] sm:w-[127px] h-[36px] sm:h-[48px] px-2 rounded-[20px] sm:rounded-[100px] ml-2 hover:brightness-75'>Add Liquidity</button>
            </div>
        </div>
        ...
        <div>
            {
                pools.map(pool => {
                    return <div key={pool.id} className={`w-full overflow-hidden ${!pool.liquidity ? 'h-[80px]' : 'h-[376px]'} mb-4 bg-[#152F30] rounded-lg px-2 sm:px-5`}>
                        <div className="w-full bg-[#152F30] h-[80px] flex items-center justify-between">
                            ...
                            <i className='cursor-pointer' onClick={() => toggleDisplayFullPool(pool.id)}>{!pool.liquidity ? <AiOutlineDown /> : <AiOutlineUp />}</i>
                        </div>
                        ...
                        <div className='mt-6 mb-7 text-[#DCDCDC]'>
                            ...
                            <div>
                                ...
                                <div className='flex justify-between'>
                                    <button className='bg-[#1B595B] text-[#69CED1] h-[48px] rounded-[100px] w-[48%]'onClick={() => dispatch(showRemoveLiquidity())}>Remove</button>
                                    <button className='bg-[#1B595B] text-[#69CED1] h-[48px] rounded-[100px] w-[48%]'>Add</button>
                                </div>
                            </div>
